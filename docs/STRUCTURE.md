...
└── processing/              # Обработка документов
    ...
    │       ├── renderer.py            # ⚠️ НЕ ИСПОЛЬЗУЕТСЯ (OCR отменён)
    │       └── ocr/                   # ⚠️ НЕ ИСПОЛЬЗУЕТСЯ (OCR отменён)
    │           ├── __init__.py
    │           ├── layout_dots.py     # LayoutTypeDotsOCR enum (для PDF)
    │           └── layout_detector.py # ⚠️ НЕ ИСПОЛЬЗУЕТСЯ для DOCX
    │
...
- ⚠️ Для DOCX OCR-подход отменён, используется только для PDF
...
6. ⚠️ **OCR-подход отменён** - не используется рендеринг и OCR для DOCX
