...
class AnnotationTool(QMainWindow):
    ...
    def init_ui(self):
        ...
        load_btn = QPushButton("📂 Загрузить PDF")
    ...
    def load_pdf(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(
            self, "Выберите файл", "", "PDF Files (*.pdf);;DOCX Files (*.docx);;All Files (*.*)"
        )
    ...
    def load_annotation_from_file(self, json_path: Path):
        ...
        try:
            ...
            if source_file and Path(source_file).exists():
                ...
                # Пытаемся найти PDF по имени файла разметки
                pdf_name = json_path.stem.replace("_annotation", "") + ".pdf"
                
                # Ищем в разных местах
                possible_paths = [
                    json_path.parent.parent / "test_files_for_metrics" / pdf_name,
                    json_path.parent / pdf_name,
                    Path("test_files_for_metrics") / pdf_name,
                    Path("experiments/metrics/test_files_for_metrics") / pdf_name,
                    Path(pdf_name),
                ]
                ...
                if not pdf_found:
                    QMessageBox.information(
                        ...
                        f"PDF файл не найден автоматически.\n"
                        f"Загрузите PDF вручную через кнопку 'Загрузить PDF'."
                    )
    ...
    def load_annotation(self):
        ...
        annotations_dir = Path("annotations")
        default_path = str(annotations_dir) if annotations_dir.exists() else ""
        
        file_path, _ = QFileDialog.getOpenFileName(
            self, "Выберите файл разметки", default_path, "JSON Files (*.json)"
        )
    ...
    def save_annotation(self):
        ...
        try:
            ...
            annotation = {
                ...
                "source_file": str(json_source_file),
