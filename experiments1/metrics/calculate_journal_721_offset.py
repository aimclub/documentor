"""
Скрипт для расчета смещения bbox для journal-10-67-5-721-729.
"""
...
def find_matching_scanned_element(
    ...
    for idx, scanned_elem in enumerate(scanned_elements):
        ...
        if pdf_type == scanned_type:
            score += 0.3
        elif pdf_type in ['text', 'header_1', 'title'] and scanned_type in ['text', 'header_1', 'title']:
            # Для title и header_1 считаем совместимыми
            if (pdf_type == 'title' and scanned_type == 'header_1') or (pdf_type == 'header_1' and scanned_type == 'title'):
                score += 0.25
            else:
                score += 0.1
        else:
...
def main():
    ...
    pdf_file = "journal-10-67-5-721-729.pdf_annotation.json"
    ...
    print("=" * 80)
    print("РАСЧЕТ СМЕЩЕНИЙ BBOX ДЛЯ journal-10-67-5-721-729")
    print("=" * 80)
    print()
