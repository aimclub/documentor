"""
Скрипт для детального анализа оглавления в Диплом.docx
"""
import zipfile
...
def analyze_docx(docx_path: Path):
    ...
    with zipfile.ZipFile(docx_path, 'r') as zip_file:
        ...
        # 1. Ищем заголовок "Содержание" или "Оглавление" (включая внутри TOC поля)
        ...
        # Также ищем "Содержание" внутри TOC поля
        if not toc_header_found:
            print("   Поиск 'Содержание' внутри TOC поля...")
            for i, para in enumerate(body.findall('w:p', NAMESPACES)):
                # Проверяем, есть ли fldChar в параграфе (TOC поле)
                fld_chars = para.findall('.//w:fldChar', NAMESPACES)
                if fld_chars:
                    text = get_paragraph_text(para)
                    if 'содержание' in text.lower() or 'оглавление' in text.lower():
                        print(f"   [OK] Найдено 'Содержание' в TOC поле на позиции {i}: '{text[:100]}'")
                        toc_header_found = True
                        toc_header_idx = i
                        break
        ...
        for i, para in enumerate(body.findall('w:p', NAMESPACES)):
            ...
            for fld_char in fld_chars_in_para:
                ...
                if fld_type == 'begin':
                    in_toc_field = True
                    toc_start_idx = i
                    print(f"   [OK] TOC поле начинается на позиции {i}")
                elif fld_type == 'end':
                    if in_toc_field:
                        toc_end_idx = i
                        print(f"   [OK] TOC поле заканчивается на позиции {i}")
                    in_toc_field = False
            
            if in_toc_field:
                text = get_paragraph_text(para)
                if text and text.strip():
                    toc_content_paras.append((i, text))
                    ...
                    print(f"   TOC content [{i}]: Hyperlink={has_hyperlink}, Page={has_page_num}, Sep={has_separators}")
...
if __name__ == '__main__':
    ...
    docx_path = test_folder / "Диплом.docx"
    if docx_path.exists():
